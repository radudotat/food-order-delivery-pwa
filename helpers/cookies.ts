...
export function setParsedCookie(key: string, value: any) {
    ...
    Cookies.set(key, JSON.stringify(value), {path: '/', expires: expiration});
}
...
export function setCookieLocation(location: GeolocationPosition) {
    ...
    if (!location.timestamp && storedLocation.timestamp && storedLocation.timestamp === location.timestamp) {
        return;
    }
